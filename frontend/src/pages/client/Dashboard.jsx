...
function RequestService() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (!serviceId) return;
    await api.post("/service-requests", { serviceId });
    setServiceId("");
  };
  useEffect(() => { load(); }, []);
  return (
    <form onSubmit={submit} className="space-x-2">
      ...
      <button className="px-3 py-2 bg-blue-600 text-white rounded">Request</button>
    </form>
  );
}
  ...
  return (
    <div className="min-h-screen grid grid-cols-5">
      <aside className="col-span-1 bg-gray-900 text-white p-4 space-y-2">
        ...
        <Link className="block px-2 py-1 rounded hover:bg-gray-800" to="">Request Service</Link>
